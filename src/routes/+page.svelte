...
<div class="relative overflow-hidden">
	...
	>
		<div class="relative w-full sm:static">
			...
			<!-- below sm, cards stack and their opaque background covers the vertically-
			 centered flight path entirely, hiding the animation; hang it off the top of
			 the headline instead. Anchoring to the headline rather than to the top of the
			 hero is what keeps the gap constant: the hero centers its content, so the
			 headline sits 40px lower on a tall phone than on a short one, and a fixed
			 offset from the top would read as a 3px gap on one and 28px on another.
			 The ink stops 15% short of the box's bottom edge (the path bottoms out at
			 viewBox y 340 of 400), so the negative margin below pulls that dead band back
			 off: 4.65vw is 15% of the box height, which is half of w-[62vw]. At sm+ cards
			 go two-column and free up the middle of the hero, so the original centered
			 placement returns. -->
			<svg
				aria-hidden="true"
				class="disc-flight pointer-events-none absolute bottom-full left-1/2 mb-[calc(-16px-4.65vw)] w-[62vw] -translate-x-1/2 sm:top-1/2 sm:bottom-auto sm:mb-0 sm:w-[min(56rem,92vw)] sm:-translate-y-1/2"
				viewBox="0 0 800 400"
				fill="none"
			>
				<defs>
			...
			<h1 class="display animate-fade-up relative mt-5 text-[clamp(3.5rem,8vw,6.5rem)] text-white">
