...
Every call goes through `src/lib/server/ai/config.ts`, which pins the model (`gemini-3.7-flash`) in one place, and uses an explicit Gemini context cache (1 hour TTL) for each ruleset's `grounding.txt` so the ~46k-token rulebook prefix isn't re-sent (and re-billed) on every request.
...
- **Local dev auth.** `ALLOW_TEST_SIGNIN=1` (set in `.dev.vars.example`) turns on an email/password test sign-in path used by Playwright and local testing, so Google OAuth credentials aren't required just to exercise signed-in behavior locally. It must never be set in a deployed environment. When it's on, a "Dev only" email/password form also appears on `/ask`'s sign-in prompt, next to the Google button — useful for testing on a phone over your LAN, where Google rejects the private-IP redirect URI a LAN address produces. The form posts to the same test endpoints Playwright uses and is gated by the identical server-side check, so it's absent whenever `ALLOW_TEST_SIGNIN` is off, including in production.
