...
The sections above are the audit record as it was measured; they are unchanged. This section records what happened after: what the unverified table turned out to hold, what shipped without coverage, what landed beyond the plan, and a defect worth carrying forward as a lesson.

**The `/admin/ai` table.** Unverified became confirmed, and worse than predicted. Seeding a conversation with a long title at 320px found two problems, not one: the table pushed the page wide, and the title link's own tap target was 17px tall against the 44px floor. Both are fixed — the table scrolls inside its own box with truncated title and user columns, and the title cell is a 44px flex row the link fills. A related defect turned up later, outside the original prediction: the page's own Previous/Next pagination links only render once there are enough conversations to paginate, and measured 31×20 — invisible to the sweep until a combined run with `admin.spec.ts` happened to seed enough rows first. Fixed the same way, gated to `pointer-coarse:`, and the sweep now seeds 31 throwaway conversations of its own so pagination is always present when it visits the route.

**The three unmeasurable fixes.** All three shipped as coded fixes with no assertion behind them, because Chromium simulates neither a retracting URL bar nor an on-screen keyboard: the landing hero moved from `vh` to `dvh`, the search dialog's result list is capped against the viewport to leave room for a keyboard, and the conversation delete button got the same touch-visibility treatment as the bookmark button (see below). They remain unverified by the harness. Anyone changing that code should check it by hand on a device rather than trust the suite to catch a regression there.

**Work beyond the plan.** Four things shipped that this spec did not anticipate:

- A dev-only email/password sign-in form (`DevSignInForm.svelte`, gated on `ALLOW_TEST_SIGNIN`), added because Google rejects private-IP redirect URIs and blocked signing in for on-device testing over LAN. Building it surfaced a real gap: `/ask` had been a dynamic route all along but was missing from `isDynamicRoute` in `hooks.server.ts`, so `locals.db`/`locals.auth` were silently never wired up for it. Fixed alongside.
- The admin bar charts gained a y-axis and a hover/tap value readout (`DailyBarChart.svelte`), after a visual pass found the existing "peak N" label plus a delayed native tooltip left a reader unable to tell what any single bar was worth.
- The unconditional 44px tap-target floor, applied broadly to satisfy the sweep, had grown small desktop pills into 44px circles and thickened the admin conversation table's rows. Gated to `pointer-coarse:` so touch keeps the floor and desktop keeps its original density.
- The mobile TOC pill moved from centered over the text to the bottom-right corner, after a visual pass found it sitting over the middle of whatever line the reader had scrolled to.

**Computed-value assertions are not visual verification.** The rule bookmark button is the clearest case. Task 7's fix set the button's `opacity` to `1`, and its test asserted exactly that — and passed. The button was reachable. But the icon inherited `text-navy/30` through `fill="currentColor"`, so it rendered at 0.3 alpha: a fully opaque button showing an all-but-invisible icon. The user found this on a real phone after the suite was green. The follow-up fix added an assertion on the icon's actual rendered color alpha against a WCAG-derived floor, not just the button's own opacity.

Two other tests on this branch passed the same way — green, but not exercising the thing they claimed to cover. The clipping-container test in `mobile-audit.spec.ts` only checked that the audit's page-level gate stayed shut, which a different, pre-existing test already covered; deleting the `isClipped` helper it was written to test would have left it green. The 16px-input test's search-dialog check clicked the Search button and scanned the DOM immediately, without waiting for the dialog portal to mount its combobox — a dialog that failed to open would have left the scan with nothing to find, and the test would have passed without covering anything. Both were rewritten to fail when the thing they test breaks, confirmed to actually fail that way, then restored.

The lesson: a test that reads a computed style, a count, or a gate value can pass while the feature it is meant to protect stays broken. It has to fail when the real thing breaks, not just when its own narrow measurement changes.

**Route coverage.** `/admin/ai/[id]` was the one route the harness didn't reach, because it needs a conversation to exist first. It is now swept at all three widths, using a seeded conversation's id, and holds every invariant.

**Cost.** The full e2e suite (98 tests, including the sweep's roughly 50 page loads) passed in 3.5 minutes locally, single-worker, measured with `time npx playwright test`. `e2e/mobile.spec.ts` alone — 20 tests — ran in 1.6 minutes, under the two-minute mark the task set for reconsidering the 768 viewport, so all three widths (320, 375, 768) stay.
